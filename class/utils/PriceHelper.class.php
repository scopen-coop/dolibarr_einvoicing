...
/**
 * \file    einvoicing/class/utils/PriceHelper.class.php
 * \ingroup einvoicing
...
class PriceHelper
{
	 ...
	 */
	public static function calculatePriceTotal($qty, $pu, $remise_percent_ligne, $txtva, $uselocaltax1_rate, $uselocaltax2_rate, $remise_percent_global, $price_base_type, $info_bits, $type, $seller = null, $localtaxes_array = [], $progress = 100, $multicurrency_tx = 1, $pu_devise = 0, $multicurrency_code = '', $forceRoundingTotalsPrecision = 'MT') // @phpstan-ignore-line
	{
		global $conf, $mysoc, $db, $hookmanager;

		$result = array();

		if (!in_array($forceRoundingTotalsPrecision, ['MT', 'MU'])) {
			$forceRoundingTotalsPrecision = 'MT';
			dol_syslog("Price.lib::calcul_price_total Warning: function is called with invalid parameter value for forceRoundingTotalsPrecision: " . $forceRoundingTotalsPrecision, LOG_WARNING);
		}
		...
		if ($uselocaltax1_rate && $apply_tax) {
			$result[14] = price2num(($tot_sans_remise_withvat * (1 + ($localtax1_rate / 100))) - $tot_sans_remise_withvat, $forceRoundingTotalsPrecision);
			$localtaxes[0] += $result[14];

			$result[9] = price2num(($tot_avec_remise_withvat * (1 + ($localtax1_rate / 100))) - $tot_avec_remise_withvat, $forceRoundingTotalsPrecision);
		...
		if ($uselocaltax2_rate && $apply_tax) {
			$result[15] = price2num(($tot_sans_remise_withvat * (1 + ($localtax2_rate / 100))) - $tot_sans_remise_withvat, $forceRoundingTotalsPrecision);
			$localtaxes[0] += $result[15];

			$result[10] = price2num(($tot_avec_remise_withvat * (1 + ($localtax2_rate / 100))) - $tot_avec_remise_withvat, $forceRoundingTotalsPrecision);
		...
		if ($price_base_type != 'TTC') {
			...
			$result[6] = price2num($tot_sans_remise, $forceRoundingTotalsPrecision);
			$result[8] = price2num($tot_sans_remise * (1 + ((($info_bits & 1) ? 0 : $txtva) / 100)) + $localtaxes[0], $forceRoundingTotalsPrecision); // Selon TVA NPR ou non
			$result8bis = price2num($tot_sans_remise * (1 + ($txtva / 100)) + $localtaxes[0], $forceRoundingTotalsPrecision); // Si TVA consideree normal (non NPR)
			$result[7] = price2num((float) $result8bis - ((float) $result[6] + $localtaxes[0]), $forceRoundingTotalsPrecision);

			$result[0] = price2num($tot_avec_remise, $forceRoundingTotalsPrecision);
			$result[2] = price2num($tot_avec_remise * (1 + ((($info_bits & 1) ? 0 : $txtva) / 100)) + $localtaxes[1], $forceRoundingTotalsPrecision); // Selon TVA NPR ou non
			$result2bis = price2num($tot_avec_remise * (1 + ($txtva / 100)) + $localtaxes[1], $forceRoundingTotalsPrecision); // Si TVA consideree normal (non NPR)
			$result[1] = price2num((float) $result2bis - ((float) $result[0] + $localtaxes[1]), $forceRoundingTotalsPrecision); // Total VAT = TTC - (HT + localtax)
			...
			$result[8] = price2num($tot_sans_remise + $localtaxes[0], $forceRoundingTotalsPrecision);
			$result[6] = price2num($tot_sans_remise / (1 + ((($info_bits & 1) ? 0 : $txtva) / 100)), $forceRoundingTotalsPrecision); // Selon TVA NPR ou non
			$result6bis = price2num($tot_sans_remise / (1 + ($txtva / 100)), $forceRoundingTotalsPrecision); // Si TVA consideree normal (non NPR)
			$result[7] = price2num((float) $result[8] - ((float) $result6bis + $localtaxes[0]), $forceRoundingTotalsPrecision);

			$result[2] = price2num((float) $tot_avec_remise + (float) $localtaxes[1], $forceRoundingTotalsPrecision);
			$result[0] = price2num((float) $tot_avec_remise / (1 + ((($info_bits & 1) ? 0 : (float) $txtva) / 100)), $forceRoundingTotalsPrecision); // Selon TVA NPR ou non
			$result0bis = price2num((float) $tot_avec_remise / (1 + ((float) $txtva / 100)), $forceRoundingTotalsPrecision); // Si TVA consideree normal (non NPR)
			$result[1] = price2num((float) $result[2] - ((float) $result0bis + (float) $localtaxes[1]), $forceRoundingTotalsPrecision); // Total VAT = TTC - (HT + localtax)
		...
		if ($uselocaltax1_rate && $apply_tax) {
			$result[14] = price2num(($tot_sans_remise_withoutvat * (1 + ($localtax1_rate / 100))) - $tot_sans_remise_withoutvat, $forceRoundingTotalsPrecision); // amount tax1 for total_ht_without_discount
			$result[8] += $result[14]; // total_ttc_without_discount + tax1

			$result[9] = price2num(($tot_avec_remise_withoutvat * (1 + ($localtax1_rate / 100))) - $tot_avec_remise_withoutvat, $forceRoundingTotalsPrecision); // amount tax1 for total_ht
		...
		if ($uselocaltax2_rate && $apply_tax) {
			$result[15] = price2num(($tot_sans_remise_withoutvat * (1 + ($localtax2_rate / 100))) - $tot_sans_remise_withoutvat, $forceRoundingTotalsPrecision); // amount tax2 for total_ht_without_discount
			$result[8] += $result[15]; // total_ttc_without_discount + tax2

			$result[10] = price2num(($tot_avec_remise_withoutvat * (1 + ($localtax2_rate / 100))) - $tot_avec_remise_withoutvat, $forceRoundingTotalsPrecision); // amount tax2 for total_ht
		...
		if (getDolGlobalString('MAIN_ROUNDING_RULE_TOT')) {
			if ($price_base_type != 'TTC') {
				$result[0] = price2num(round((float) $result[0] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[1] = price2num(round((float) $result[1] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[9] = price2num(round((float) $result[9] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[10] = price2num(round((float) $result[10] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[2] = price2num((float) $result[0] + (float) $result[1] + (float) $result[9] + (float) $result[10], $forceRoundingTotalsPrecision);
			} else {
				$result[1] = price2num(round((float) $result[1] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[2] = price2num(round((float) $result[2] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[9] = price2num(round((float) $result[9] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[10] = price2num(round((float) $result[10] / getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), 0) * getDolGlobalFloat('MAIN_ROUNDING_RULE_TOT'), $forceRoundingTotalsPrecision);
				$result[0] = price2num((float) $result[2] - (float) $result[1] - (float) $result[9] - (float) $result[10], $forceRoundingTotalsPrecision);
			}
		}
